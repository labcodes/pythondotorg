...
class EventForm(forms.Form):
    event_name = forms.CharField(
        widget=set_placeholder(
            "Name of the event (including the user group name for " "user group events)"
        )
    )
    event_type = forms.CharField(
        widget=set_placeholder("conference, bar camp, sprint, user group meeting, etc.")
    )
    ...
    location = forms.CharField(widget=set_placeholder("IFEMA building, Madrid, Spain"))
    ...
    recurrence = forms.CharField(
        widget=set_placeholder("None, every second Thursday, monthly, etc.")
    )
    ...
    def send_email(self, creator):
        ...
        send_mail(
            subject='New event submission: "{}"'.format(
