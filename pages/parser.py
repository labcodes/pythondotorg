...
def read_content_file(dirpath):
    ...
    if os.path.exists(c_ht):
        with open(c_ht, "rb") as c_ht_file:
            raw_input = c_ht_file.read()
        detection = chardet.detect(raw_input)

        with open(c_ht, encoding=detection["encoding"], errors="ignore") as file_input:
            msg = email.message_from_file(file_input)

        filename = c_ht

    elif os.path.exists(c_rst):
        with open(c_rst, "rb") as c_rst_file:
            rst_text = c_rst_file.read()
        rst_content_text = rst_text.lstrip()
