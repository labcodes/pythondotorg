...
class SponsorshipModelTests(TestCase):
    ...
    def test_create_new_sponsorship(self):
        ...
        for benefit in self.benefits:
            ...
            self.assertEqual(
                sponsor_benefit.benefit_internal_value,
                benefit.internal_value,
            )
    ...
    def test_create_new_sponsorship_with_package(self):
        ...
        self.assertEqual(
            sponsorship.agreed_fee, 100
        )  # can display the price because there's not customizations
    ...
    def test_create_new_sponsorship_with_package_modifications(self):
        ...
        self.assertIsNone(
            sponsorship.agreed_fee
        )  # can't display the price with customizations
        for benefit in sponsorship.benefits.all():
            self.assertFalse(benefit.added_by_user)
    ...
    def test_rollback_approved_sponsorship_with_contract_should_delete_it(
        self,
    ):
        sponsorship = Sponsorship.new(self.sponsor, self.benefits)
    ...
    def test_can_not_rollback_sponsorship_to_edit_if_contract_was_sent(
        self,
    ):
        sponsorship = Sponsorship.new(self.sponsor, self.benefits)
        sponsorship.status = Sponsorship.APPROVED
    ...
    def test_raise_exception_when_trying_to_create_sponsorship_for_same_sponsor(
        self,
    ):
        sponsorship = Sponsorship.new(self.sponsor, self.benefits)
        finalized_status = [
    ...
    def test_display_agreed_fee_for_approved_and_finalized_status(
        self,
    ):
        sponsorship = Sponsorship.new(self.sponsor, self.benefits)
...
class SponsorshipCurrentYearTests(TestCase):
    ...
    def test_singleton_object_cannot_be_deleted(self):
        ...
        self.assertIn(
            "Singleton object cannot be delete. Try updating it instead.",
            str(context.exception),
        )
...
class SponsorshipPackageTests(TestCase):
    ...
    def test_has_user_customization_if_benefit_from_other_package(
        self,
    ):
        ...
        self.assertEqual(
            customization,
            self.package.get_user_customization(benefits),
        )
    ...
    def test_no_user_customization_if_all_benefits_from_package(self):
        ...
        self.assertEqual(
            customization,
            self.package.get_user_customization(self.package_benefits),
        )
    ...
    def test_has_user_customization_if_missing_package_benefit(self):
        ...
        customization = {
            "added_by_user": set(),
            "removed_by_user": {removed_benefit},
        }
        self.assertTrue(has_customization)
        self.assertEqual(
            customization,
    ...
    def test_no_user_customization_if_at_least_one_of_conflicts_is_passed(
        self,
    ):
    ...
    def test_user_customization_if_missing_benefit_with_conflict(
        self,
    ):
        benefits = baker.make(SponsorshipBenefit, _quantity=3)
        benefits[0].conflicts.add(benefits[1])
...
- PSF - {b3.name} [^3]"""
    ...
    def test_raise_invalid_status_when_trying_to_execute_contract_if_not_awaiting_signature(
        self,
    ):
        contract = baker.make_recipe(
    ...
    def test_raise_invalid_status_when_trying_to_nullify_contract_if_not_awaiting_signature(
        self,
    ):
        contract = baker.make_recipe(
...
class SponsorBenefitModelTests(TestCase):
    ...
    def test_reset_attributes_updates_all_basic_information(self):
        benefit = baker.make(
            SponsorBenefit,
            sponsorship_benefit=self.sponsorship_benefit,
        )
    ...
    def test_reset_attributes_recreate_features_but_keeping_previous_values(
        self,
    ):
        cfg = RequiredTextAssetConfiguration.objects.create(
            benefit=self.sponsorship_benefit,
    ...
    def test_clone_related_objects_as_well(self):
        ...
        self.assertEqual(
            2023,
            benefit_2023.packages.values_list("year", flat=True).distinct().first(),
        )
    ...
    def test_clone_benefit_feature_configurations(self):
        ...
        self.assertEqual(
            1,
            RequiredTextAssetConfiguration.objects.filter(benefit=benefit_2023).count(),
        )
        self.assertEqual(
            1,
            RequiredTextAssetConfiguration.objects.filter(benefit=benefit_2023).count(),
...
class SponsorEmailNotificationTemplateTests(TestCase):
    ...
    def test_map_sponsorship_info_to_simplified_context_data(self):
        ...
        context = self.notification.get_email_context_data(
            sponsorship=self.sponsorship, extra="foo"
        )
...
class TieredBenefitConfigurationModelTests(TestCase):
    ...
    def test_do_not_return_feature_if_benefit_from_other_package(
        self,
    ):
    ...
    def test_display_modifier_only_modifies_name_if_same_package(
        self,
    ):
        name = "Benefit"
...
class RequiredImgAssetConfigurationTests(TestCase):
    def setUp(self):
        self.sponsor_benefit = baker.make(
            SponsorBenefit, sponsorship__sponsor__name="Foo"
        )
        self.config = baker.make(
            RequiredImgAssetConfiguration,
            related_to=AssetsRelatedTo.SPONSOR.value,
    ...
    def test_get_benefit_feature_respecting_configuration(self):
        benefit_feature = self.config.get_benefit_feature(
            sponsor_benefit=self.sponsor_benefit
        )
    ...
    def test_create_benefit_feature_and_sponsor_generic_img_assets(
        self,
    ):
    ...
    def test_clone_configuration_for_new_sponsorship_benefit_without_due_date(
        self,
    ):
...
class RequiredTextAssetConfigurationTests(TestCase):
    ...
    def test_create_benefit_feature_and_sponsor_generic_text_asset(
        self,
    ):
        sponsor = self.sponsor_benefit.sponsorship.sponsor
    ...
    def test_clone_configuration_for_new_sponsorship_benefit_with_new_due_date(
        self,
    ):
        sp_benefit = baker.make(SponsorshipBenefit, year=2023)
...
class RequiredTextAssetTests(TestCase):
    ...
    def test_required_asset_class_inherits_from_expected_classed(
        self,
    ):
        classes = (
            RequiredAssetMixin,
