...
class LogoPlacementeAPIListTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.permission = Permission.objects.get(
            name="Can access sponsor placement API"
        )
        ...
        sponsorships = baker.make_recipe(
            "sponsors.tests.finalized_sponsorship",
            sponsor=iter(self.sponsors),
            _quantity=3,
        )
        self.sp1, self.sp2, self.sp3 = sponsorships
        baker.make_recipe(
            "sponsors.tests.logo_at_download_feature",
            sponsor_benefit__sponsorship=self.sp1,
        )
        baker.make_recipe(
            "sponsors.tests.logo_at_sponsors_feature",
            sponsor_benefit__sponsorship=self.sp1,
        )
        baker.make_recipe(
            "sponsors.tests.logo_at_sponsors_feature",
            sponsor_benefit__sponsorship=self.sp2,
        )
        baker.make_recipe(
        ...
        self.assertEqual(
            ...
            len(
                [
                    p
                    for p in data
                    if p["flight"] == LogoPlacementChoices.SPONSORS_PAGE.value
                ]
            ),
        )
        self.assertEqual(
            ...
            len(
                [
                    p
                    for p in data
                    if p["flight"] == LogoPlacementChoices.DOWNLOAD_PAGE.value
                ]
            ),
        )
        self.assertEqual(
            1,
            len([p for p in data if p["flight"] == LogoPlacementChoices.SIDEBAR.value]),
        )
        self.assertEqual(
            2,
            len([p for p in data if p["sponsor"] == self.sponsors[0].name]),
        )
        self.assertEqual(
            1,
            len([p for p in data if p["sponsor"] == self.sponsors[1].name]),
        )
        self.assertEqual(
            1,
            len([p for p in data if p["sponsor"] == self.sponsors[2].name]),
        )
        self.assertEqual(
            None,
            [p for p in data if p["publisher"] == PublisherChoices.FOUNDATION.value][0][
                "sponsor_url"
            ],
        )
        self.assertEqual(
            f"http://testserver/psf/sponsors/#{slugify(self.sp3.sponsor.name)}",
            [p for p in data if p["publisher"] == PublisherChoices.PYPI.value][0][
                "sponsor_url"
            ],
        )
        self.assertCountEqual(
            [
                self.sp1.sponsor.description,
                self.sp1.sponsor.description,
                self.sp2.sponsor.description,
            ],
            [
                p["description"]
                for p in data
                if p["publisher"] == PublisherChoices.FOUNDATION.value
            ],
        )
        self.assertEqual(
            [
            ...
            [
                p["description"]
                for p in data
                if p["publisher"] == PublisherChoices.PYPI.value
            ],
        )
...
class SponsorshipAssetsAPIListTests(APITestCase):
    ...
    def test_enable_to_filter_by_assets_with_no_value_via_querystring(
        self,
    ):
    ...
    def test_serialize_img_value_as_url_to_image(self):
        ...
        url = (
            reverse_lazy("assets_list")
            + f"?internal_name={self.img_asset.internal_name}"
        )
        response = self.client.get(url, HTTP_AUTHORIZATION=self.authorization)
        data = response.json()
