...
class SponsorshipQuerySetTests(TestCase):
    ...
    def test_filter_sponsorship_with_logo_placement_benefits(self):
        sponsorship_with_download_logo = baker.make_recipe(
            'sponsors.tests.finalized_sponsorship'
        )
        sponsorship_with_sponsors_logo = baker.make_recipe(
            'sponsors.tests.finalized_sponsorship'
        )
        simple_sponsorship = baker.make_recipe('sponsors.tests.finalized_sponsorship')

        download_logo_benefit = baker.make(
            SponsorBenefit, sponsorship=sponsorship_with_download_logo
        )
        baker.make_recipe(
            'sponsors.tests.logo_at_download_feature',
            sponsor_benefit=download_logo_benefit,
        )
        sponsors_logo_benefit = baker.make(
            SponsorBenefit, sponsorship=sponsorship_with_sponsors_logo
        )
        baker.make_recipe(
            'sponsors.tests.logo_at_sponsors_feature',
            sponsor_benefit=sponsors_logo_benefit,
        )
    ...
    def test_filter_sponsorship_by_benefit_feature_type(self):
        sponsorship_feature_1 = baker.make_recipe(
            'sponsors.tests.finalized_sponsorship'
        )
        sponsorship_feature_2 = baker.make_recipe(
            'sponsors.tests.finalized_sponsorship'
        )
        baker.make(
            LogoPlacement,
            sponsor_benefit__sponsorship=sponsorship_feature_1,
        )
        baker.make(
            TieredBenefit,
            sponsor_benefit__sponsorship=sponsorship_feature_2,
        )
...
class SponsorshipBenefitManagerTests(TestCase):
    def setUp(self):
        ...
        self.regular_benefit_unavailable = baker.make(
            SponsorshipBenefit, year=current_year, unavailable=True
        )
        self.regular_benefit.packages.add(package)
        self.regular_benefit.packages.add(package)
        self.a_la_carte = baker.make(SponsorshipBenefit, year=current_year - 1)
        self.a_la_carte_unavail = baker.make(
            SponsorshipBenefit,
            year=current_year - 1,
            unavailable=True,
        )
        self.standalone = baker.make(SponsorshipBenefit, standalone=True)
        self.standalone_unavail = baker.make(
            SponsorshipBenefit, standalone=True, unavailable=True
        )
